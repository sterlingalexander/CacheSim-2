...
int findInDirectory(addr, directory)  {
   for (int i = 0, i < DIRECTORY_SIZE; i++)
      if (directory[i].tag == addr)
         return i;

   return -1;   
}

int getUnownedPos(vector<FBV> directory)  {
   for (int i = 0; i < DIRECTORY_SIZE; i++)
      if (directory[i].Flags == UNOWNED)
         return i;
   cout << "DIRECTORY FULL....HANDLE THE CORNER CASE\n\n";
}
